...
public class PetsshopIntegrationTest {
	...
	void testCreate() throws Exception {
		...
		RequestBuilder request = post("/createAnimal").contentType(MediaType.APPLICATION_JSON).content(requestBody);
	...
	void testUpdate() throws Exception {
		Animal newAnimal = new Animal("Fluffy", "German Shepherd", "Dog", 500, "Male", "Brown", "01/12/12");
		String requestBody = this.mapper.writeValueAsString(newAnimal);
		RequestBuilder request = put("/updateAnimal?id=1").contentType(MediaType.APPLICATION_JSON).content(requestBody);
	...
	void testRead() throws Exception {
		...
		this.mockMVC.perform(get("/getAnimalAll")).andExpect(status().isOk()).andExpect(content().json(responseBody));
	}
